...
import { 
  ...
  Send, UserCheck, Eye, TrendingUp, ChevronRight, Zap, CheckSquare, Activity, Users, X, MoreHorizontal, AlertTriangle, LogOut
} from "lucide-react";
      ...
      <div className="lg:col-span-2 space-y-6">
              ...
              {isOwnProfile ? (
                <>
                  ...
                  <button 
                    ...
                    className="bg-rose-600 hover:bg-rose-700 border border-rose-500 text-rose-50 font-bold text-[10px] sm:text-xs px-3 py-1.5 rounded-lg transition-all shadow-md cursor-pointer flex items-center gap-1.5"
                  >
                  ...
                  {!isOwnProfile && (
                    <div className="flex flex-col gap-3 pt-1.5 border-t border-slate-850/60 mt-3.5">
                      <div className="flex flex-wrap items-center gap-2.5">
                        ...
                        {/* 3. MESSAGE BUTTON (LinkedIn Style: only show when connected, otherwise hide) */}
                        {connectionStatus === "Connected" && (
                          <button 
                            onClick={() => {
                            ...
                            className="px-4 py-2 bg-slate-900 border border-slate-800 hover:border-indigo-500/40 text-slate-200 rounded-xl text-xs font-bold flex items-center gap-1.5 cursor-pointer transition-all"
                          >
        ...
        {activeSubTab === "portfolio" && (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-tr from-slate-900 to-indigo-950/20 border border-slate-850 p-5 rounded-2xl flex flex-col md:flex-row gap-5 items-center justify-between">
              <div className="space-y-1.5 text-left">
                <span className="text-[9.5px] font-mono text-emerald-400 font-extrabold uppercase tracking-wider block">Profile Health Metrics</span>
                <h4 className="font-extrabold text-white text-sm">Professional Profile Completeness</h4>
                <p className="text-[11px] text-slate-400 max-w-sm leading-relaxed">
                  Completing your verification checklists unlocks advanced campus visibility filters and matches your resume block to corporate placement scouts.
                </p>
                
                {/* Micro checklist metrics */}
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 pt-2 text-[10px] font-sans text-slate-300">
                  <div className="flex items-center gap-1.5">
                    <CheckSquare className="w-3.5 h-3.5 text-emerald-400" />
                    <span>Executive bio linked</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <CheckSquare className="w-3.5 h-3.5 text-emerald-400" />
                    <span>Academic node active</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <CheckSquare className="w-3.5 h-3.5 text-emerald-400" />
                    <span>Skills matrix endorsed</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <CheckSquare className="w-3.5 h-3.5 text-indigo-400" />
                    <span className="text-indigo-300">Pinned Portfolio connected</span>
                  </div>
                </div>
              </div>

              {/* Progress visual ring */}
              <div className="flex flex-col items-center justify-center shrink-0 p-1 bg-slate-950/50 border border-slate-850/60 rounded-xl w-32 h-32 relative">
                <span className="text-2xl font-mono font-extrabold text-emerald-400">92%</span>
                <span className="text-[8.5px] font-mono text-slate-550 uppercase tracking-widest mt-1 block">Score Rank</span>
                <div className="absolute inset-2 border-2 border-emerald-400/20 rounded-lg pointer-events-none" />
            ...
            <div className="bg-slate-900 border border-slate-850 p-5 rounded-2xl space-y-3.5 text-left">
              <h3 className="text-[11px] font-bold font-mono text-indigo-400 uppercase tracking-widest flex items-center justify-between">
                <span className="flex items-center gap-1.5">
                  <BookOpen className="w-4 h-4 text-indigo-500" /> Core Skills Inventory & Verified Endorsements
                </span>
                <span className="text-[8px] text-slate-500 italic">Click skill tag to endorse</span>
              </h3>
              ...
              <div className="flex flex-wrap gap-2.5">
                {(skillsStr ? skillsStr.split(",") : ["React", "TypeScript", "Node.js"]).map((sk, idx) => {
                  ...
                  // Setup pseudo-random seed endorsement votes count based on char code
                  const baseSeed = cleaned.charCodeAt(0) % 7 + 3;
                  return (
                    <button 
                      key={idx} 
                      onClick={() => alert(`You have successfully endorsed ${cleaned} for ${user.fullName || "this student collaborator"}!`)}
                      className="px-3 py-1.5 bg-slate-950 border border-slate-800 hover:border-indigo-500/55 hover:bg-slate-900/50 text-xs font-semibold text-slate-300 rounded-xl transition-all flex items-center gap-2 group cursor-pointer focus:outline-none"
                    >
                      <Zap className="w-3 h-3 text-indigo-400 group-hover:text-indigo-300 transition-colors" />
                      <span>{cleaned}</span>
                      <span className="bg-indigo-950 text-indigo-300 px-1.5 py-0.5 rounded text-[9px] font-mono font-bold group-hover:bg-indigo-900 transition-colors">
                        +{baseSeed}
                      </span>
                      <ShieldCheck className="w-3 h-3 text-emerald-400 ml-0.5" title="Skills Vetted" />
                    </button>
                  );
                })}
              </div>
